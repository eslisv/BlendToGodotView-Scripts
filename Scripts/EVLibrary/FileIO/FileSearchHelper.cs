...
namespace EVLibrary.FileIO
{
    ...
    {
        public static IReadOnlyCollection<string> SearchPathForFilesWithExtension(string path, string searchPattern = "*", SearchOption searchOption = SearchOption.AllDirectories)
        {
            if (!Directory.Exists(path)) { return null; }
            List<string> result = new List<string>();
            Directory.GetFiles(path, searchPattern, searchOption).ToList().ForEach(path =>
            {
                if (path.EndsWith(Path.GetExtension(searchPattern))) // If searchpattern is *.blend it grabs only .blend files (the '*' causes errors)
                {
                    result.Add(path.StandardizeSlashesToBackslash());
                }
            });
        ...
        public static string FindExecutableInstallPath(string executableName)
        {
            if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
            {
                ...
                string[] libraryFoldersFile = SysFile.ReadAllLines(steamInstallPath + @"\libraryfolders.vdf");
